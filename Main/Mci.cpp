...
//
// Modified Open() function by Calvin Tong (9/26/02)
...
DWORD CMciDevice::Open(LPCSTR lpstrDrive, BOOL bShareable /*=FALSE*/) 
{	
	ASSERT(lpstrDrive != NULL);

	MCIERROR	nErr;
	char		szCommandString[256];
	char		szReturnDeviceID[20];
	int			nCDROMAliasNo = 1;
	TCHAR		szDeviceName[256];

RetryOpenDevice:

	wsprintf(szCommandString, "open %s type cdaudio alias cd%d wait", lpstrDrive, nCDROMAliasNo);
	if (bShareable == TRUE)
	{
		lstrcat(szCommandString, " shareable");
	}
	nErr = mciSendString(szCommandString, szReturnDeviceID, 20, 0);

	if (nErr == 289)
	{
		// Failed. Already use the same alias
		nCDROMAliasNo++;

		// retry 5 times more only
		if (m_nOpenRetryCnt > 0)
		{
			// Decrement retry count
			m_nOpenRetryCnt--;

			goto RetryOpenDevice;
		}
	}

	if (!nErr)
	{
		m_wDeviceID = atoi(szReturnDeviceID);
		if (m_wDeviceID <= 0)
		{
			m_wDeviceID = 0;
		}

		if (QueryDosDevice(lpstrDrive, szDeviceName, 256))
		{
			lstrcpy(m_szDeviceName, szDeviceName);
		}
		else
		{
			m_szDeviceName[0] = 0;
...
void CMciDevice::GetDeviceName(TCHAR *szDeviceName)
{
	lstrcpy(szDeviceName, m_szDeviceName);
}
